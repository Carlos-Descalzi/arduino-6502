...
register unsigned char reg_a asm("r2");
register unsigned char reg_x asm("r3");
register unsigned char reg_y asm("r4");
register unsigned char reg_p asm("r5");
register unsigned char reg_s asm("r6");
...
void op_brk(){
	taddress ret_address = pc;
	ret_address.w+=1;
	reg_p |= FLAG_B;
  handle_interrupt(0xFFFE,ret_address,reg_p/*|FLAG_B*/);
}
...
void op_adc_##mode(){\
    ...
    unsigned short v;\
    if (reg_p & FLAG_D){\
			uint8_t l = (reg_a & 0xf) + (r &0xf) + (reg_p & FLAG_C ? 1 : 0);\
			if ((l & 0xFF) > 9) l+=6; \
			uint8_t h = (reg_a >> 4) + (r >> 4) + (l > 15 ? 1 : 0);\
			if (h > 9) h+=6;\
			v = ((l & 0xf) | (h << 4)) & 0xFF;\
      update_c(h > 15);\
			reg_p &= ~FLAG_V;\
...
void op_bit_##mode(){\
    unsigned char r = fetch_##mode();\
    unsigned char c = reg_a & r;\
    if (r & 0x80)  reg_p |= FLAG_N; else reg_p &= ~FLAG_N;\
    if (r & 0x40)  reg_p |= FLAG_V; else reg_p &= ~FLAG_V;\
...
void op_sbc_##mode(){\
    ...
    short v;\
    if (reg_p & FLAG_D){\
			uint8_t l = (reg_a & 0xf) - (c & 0xf) - (reg_p & FLAG_C ? 0 : 1);\
			if ((l & 0x10) != 0) l-=6;\
			uint8_t h = (reg_a >> 4) - (c >> 4) - ((l & 0x10) != 0 ? 1 : 0);\
			if ((h & 0x10) != 0) h-=6;\
      v = (l & 0xf) | ((h << 4) & 0xFF);\
      reg_p &= ~FLAG_V;\
      update_c((h & 0xFF) < 15);\
      ...
      v = reg_a - c;\
      if (!(reg_p & FLAG_C)) v--;\
      if (!(v & 0x80) && (reg_a & 0x80)) reg_p |= FLAG_V; else reg_p &= ~FLAG_V;\
			update_c(v >= 0);\
    }\
